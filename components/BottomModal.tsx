...
import { TrueSheet, TrueSheetProps, DetentChangeEvent } from '@lodev09/react-native-true-sheet';
import { Keyboard, Image, StyleSheet, View, Pressable, Platform, GestureResponderEvent, Text } from 'react-native';
import SaveFileButton from './SaveFileButton';
import { useTheme } from './themes';
import { Icon } from '@rneui/base';

// Sheet size can be a number (0-1 for percentage, >1 for pixels), 'auto', 'small', 'medium', 'large'
type SheetSize = number | 'auto' | 'small' | 'medium' | 'large';
...
interface BottomModalProps extends Omit<TrueSheetProps, 'header' | 'footer'> {
  ...
  onPresent?: () => void;
  onSizeChange?: (event: DetentChangeEvent) => void;
...
const BottomModal = forwardRef<BottomModalHandle, BottomModalProps>(
  (
    ...
    return (
      <TrueSheet
        ref={trueSheetRef}
        detents={detents as any}
        onDidDismiss={onClose}
        onDidPresent={onPresent}
        onDetentChange={onSizeChange}
