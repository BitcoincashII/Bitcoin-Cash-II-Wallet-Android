...
import org.bch2.wallet.components.segmentedcontrol.CustomSegmentedControlPackage
...
class MainApplication : Application(), ReactApplication {
    ...
    override fun onCreate() {
        super.onCreate()
        sharedPref = getSharedPreferences("group.org.bch2.wallet", Context.MODE_PRIVATE)
    ...
    private fun initializeBugsnag() {
        ...
        if (isDoNotTrackEnabled != "1") {
            try {
                ...
                val enabled = ai.metaData?.getBoolean("com.bugsnag.android.ENABLED", false) ?: false
                if (enabled) {
                    Bugsnag.start(this)
                } else {
